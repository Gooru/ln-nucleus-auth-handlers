...
public final class MessageResponseFactory {
  ...
  public static MessageResponse createConflictRespose(JsonObject errors) {
    return new MessageResponse.Builder().failed().setStatusHttpCode(HttpStatus.CONFLICT)
        .setResponseBody(errors).build();
  }

  public static MessageResponse createPostResponse(String location) {
    return new MessageResponse.Builder().successful().setHeader("Location", location)
        .setStatusCreated().build();
  }
  ...
  public static MessageResponse createSeeOtherResponse(String redirectUrl) {
    return new MessageResponse.Builder().successful().setStatusHttpCode(HttpStatus.SEE_OTHER)
        .setHeader(HttpConstants.HEADER_CONTENT_LENGTH, "0")
        .setHeader(HttpConstants.HEADER_LOCATION, redirectUrl).build();
  }
  ...
  public static MessageResponse createMovePermanentlyResponse(String redirectUrl) {
    return new MessageResponse.Builder().successful()
        .setStatusHttpCode(HttpStatus.MOVED_PERMANENTLY)
        .setHeader(HttpConstants.HEADER_CONTENT_LENGTH, "0")
        .setHeader(HttpConstants.HEADER_LOCATION, redirectUrl).build();
  }
}
