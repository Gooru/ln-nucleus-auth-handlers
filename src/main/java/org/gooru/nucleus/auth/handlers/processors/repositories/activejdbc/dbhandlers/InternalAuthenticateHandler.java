...
public class InternalAuthenticateHandler implements DBHandler {
  ...
  private static final List<String> USER_DEMOGRAPHIC_FIELDS = Arrays.asList("first_name",
      "last_name", "user_category", "birth_date", "grade", "course", "thumbnail", "gender", "about",
      "school_id", "school", "school_district_id", "school_district", "email", "country_id",
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    LazyList<AJEntityUsers> users = AJEntityUsers.findBySQL(AJEntityUsers.SELECT_FOR_SIGNIN,
        username, username, tenant.getString(AJEntityTenant.ID));
    if (users.isEmpty()) {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    JsonObject response = new JsonObject(
        JsonFormatterBuilder.buildSimpleJsonFormatter(false, USER_DEMOGRAPHIC_FIELDS).toJson(user));
    ...
    response.put(ParameterConstants.PARAM_CDN_URLS,
    ...
    final String token =
        InternalHelper.generateToken(user.getString(AJEntityUsers.ID), partnerId, clientId);
