...
public class AuthorizeUserHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (isPartner) {
      users = AJEntityUsers.findBySQL(AJEntityUsers.SELECT_BY_EMAIL_PARTNER_ID,
          identityId.toLowerCase(), partnerId);
    } else {
      users = AJEntityUsers.findBySQL(AJEntityUsers.SELECT_BY_EMAIL_TENANT_ID,
          identityId.toLowerCase(), tenantId);
    }

    EventBuilder eb;
    if (users.isEmpty()) {
      LOGGER.debug("user not found in database for email '{}', tenant '{}', partner '{}'",
          identityId, tenantId, partnerId);
      ...
      user.setString(AJEntityUsers.EMAIL,
          userJson.getString(ParameterConstants.PARAM_IDENTITY_ID).toLowerCase());
    ...
    return new ExecutionResult<>(MessageResponseFactory.createPostResponse(result, eb),
