...
public class InternalOAuth2SSOHandler implements DBHandler {

  private final ProcessorContext context;
  private static final Logger LOGGER = LoggerFactory.getLogger(InternalOAuth2SSOHandler.class);
  ...
  public InternalOAuth2SSOHandler(ProcessorContext context) {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    JsonObject errors = new DefaultPayloadValidator().validatePayload(context.requestBody(),
        RequestValidator.oauth2ssoFieldSelector(), RequestValidator.getValidatorRegistry());
    ...
    if (!grantType.equalsIgnoreCase(HelperConstants.GrantTypes.oauth2.getType())) {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (partners.isEmpty()) {
      tenants = AJEntityTenant.findBySQL(AJEntityTenant.SELECT_BY_ID_SECRET, clientId,
          InternalHelper.encryptClientKey(clientKey), HelperConstants.GrantTypes.oauth2.getType());
    } else {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (users.isEmpty()) {
      ...
      user.setString(AJEntityUsers.LOGIN_TYPE, HelperConstants.UserLoginType.oauth2.getType());
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createPostResponse(result,
              EventBuilderFactory.getOAuth2SSOSigninEventBuilder(user.getString(AJEntityUsers.ID))),
