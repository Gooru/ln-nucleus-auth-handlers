...
public class AJInternalRepo implements InternalRepo {
    ...
    public MessageResponse authenticate() {
        return new TransactionExecutor().executeTransaction(DBHandlerBuilder.buildInternalAuthenticateHandler(context));
    }

    @Override
    public MessageResponse impersonate() {
        return new TransactionExecutor().executeTransaction(DBHandlerBuilder.buildInternalImpersonateHadler(context));
    }
